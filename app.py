...
def recognize(imageToBeTested):
    img_ori = cv2.imread(imageToBeTested)
    ...
    def find_chars(contour_list):
        ...
        for d1 in contour_list:
            ...
            for d2 in contour_list:
                ...
                if distance < diagonal_length1 * 5 \
    ...
    json_file = open('model_LicensePlate.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    model = model_from_json(loaded_model_json)
    model.load_weights("model_LicensePlate.h5")
    ...
    def show_results(model):
        ...
        return plate_number
    result = show_results(model)
    if result=='':
        return "Not Recognized try another image"
    else:
        return result
...
def uploadImage():
    if request.method == 'POST': # Just to Validate if user is uploading the file in POST Request
        ...
        label = recognize(file.filename)

        return render_template('recognized.html', name=label)
